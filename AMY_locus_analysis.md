...
`scripts/amy_haplotype_tags.py` reads the W lines of the subgraph and the
...
`scripts/plot_haplotypes.R` draws the tree next to the gene structures with
`ape` and `gggenes`.
...
`scripts/msa_to_vcf.py` turns the alignment into a VCF of substitutions, with
...
`scripts/hudson_fst.py`. It is also a ratio of averages and is robust to
